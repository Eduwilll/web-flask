...
whatsbot = Blueprint('whatsbot', __name__)

@whatsbot.route("/whatsbot", methods=["get", "post"])
def reply():
    ...
    # Chegando o Status do user, estados possiveis para user = main, ordering, address, ordered.
        ...
        if option == 1:
            ...
            res.message(
                "Para conhecer nosso produtos digite:\n\n1️⃣ *Produtos*\n2️⃣ *Combos e Kits*\n3️⃣ *Promoções*\n4️⃣*Retorna* ")
        ...
        complaints.insert_one(
            {"number": number, "complaint": text, "name": profileName, "complaint_time": datetime.now()})
        ...
        res.message(
            f"Oi, obrigado por nos contatar novamente *{profileName.capitalize()}*!😘\n Você pode escolher uma das opções abaixo\n\n *Digite* o numero correspodente:\n\n1️⃣  Para saber nosso *endereço* e *horário*. \n2️⃣  Para conhecer nosso menu de *salgados*.\n3️⃣  Para saber nossa formas de *pagamento*.\n4️⃣  Para saber a taxa de *entrega*. \n5️⃣ Fazer Reclamação ou Ajuda \n6️⃣ Finalizar")
        users.update_one({"number": number}, {"$set": {"status": "main"}})
